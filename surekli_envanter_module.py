...
# ==================== İÇ HIRSIZLIK ANALİZİ (SÜREKLİ ENVANTER) ====================

def detect_ic_hirsizlik_surekli(df, df_onceki=None):
    """
    Sürekli envanter için iç hırsızlık tespiti
    Kural: Fark + İptal = 0 ise şüpheli (iptal ile kapatılmış açık)
    """
    results = []
    
    for idx, row in df.iterrows():
        satis_fiyati = row.get('Satış Fiyatı', 0) or row.get('Birim Fiyat', 0) or 0
        if satis_fiyati < 100:
            continue
        
        fark_miktari = row.get('Fark Miktarı', 0) or 0
        kismi = row.get('Kısmi Envanter Miktarı', 0) or 0
        onceki_fark = row.get('Önceki Fark Miktarı', 0) or 0
        iptal_tutar = row.get('İptal Satır Tutarı', 0) or 0
        
        toplam = fark_miktari + kismi + onceki_fark
        
        if toplam >= 0 or iptal_tutar <= 0:
            continue
        
        # İptal adet = İptal Tutarı / Satış Fiyatı
        iptal_adet = iptal_tutar / satis_fiyati if satis_fiyati > 0 else 0
        
        # Hesaplanan fark = Toplam + İptal Adet
        hesaplanan_fark = abs(toplam) - iptal_adet
        
        # Risk belirleme
        if abs(hesaplanan_fark) < 0.1:
            risk = "🔴 ÇOK YÜKSEK"
            durum = "TAM EŞİT"
        elif abs(hesaplanan_fark) <= 1:
            risk = "🟠 YÜKSEK"
            durum = "YAKIN (±1)"
        elif abs(hesaplanan_fark) <= 2:
            risk = "🟡 ORTA"
            durum = "YAKIN (±2)"
        else:
            continue
        ...
        # Ürün grubu bul
        urun_grubu = ''
        for col in ['Ürün Grubu Tanımı', 'Mal Grubu Tanımı', 'Ürün Grubu', 'Ana Grup']:
        ...
        results.append({
            ...
            'TOPLAM': toplam,
            'İptal Satır': iptal_adet,
            'Fark': hesaplanan_fark,
            'Durum': durum,
    ...
    if len(result_df) > 0:
        ...
        risk_order = {'🔴 ÇOK YÜKSEK': 0, '🟠 YÜKSEK': 1, '🟡 ORTA': 2}
...
def enrich_with_camera_surekli(df, get_iptal_func, magaza_kodu, full_df, kamera_gun=15):
    """
    Sürekli envanter iç hırsızlık tablosuna kamera kontrol bilgisi ekler
    get_iptal_func: (magaza_kodu, malzeme_kodlari) -> dict fonksiyonu
    """
    ...
    malzeme_kodlari = result_df['Malzeme Kodu'].astype(str).tolist()
    
    # Kategori bilgisi
    kategori_col = None
    for col in ['Ürün Grubu', 'Mal Grubu Tanımı', 'Ana Grup']:
        ...
        for _, row in result_df.iterrows():
            ...
            if kategori and kategori not in kategori_urunleri:
                if kategori_col in full_df.columns:
                    fiyat_col = 'Satış Fiyatı' if 'Satış Fiyatı' in full_df.columns else 'Birim Fiyat'
                    if fiyat_col in full_df.columns:
                        price = pd.to_numeric(full_df[fiyat_col], errors='coerce').fillna(0)
    ...
    iptal_data = get_iptal_func(magaza_kodu, tum_kodlar)
    ...
    for _, row in result_df.iterrows():
        ...
        # Önce ürünün kendi iptaline bak
        if malzeme_kodu in iptal_data and iptal_data[malzeme_kodu]:
            iptaller = iptal_data[malzeme_kodu]
            son_iptal = iptaller[0] if iptaller else None
            if son_iptal:
                tarih = son_iptal.get('tarih', '')
                saat = str(son_iptal.get('saat', ''))[:8]
                kasa_no = str(son_iptal.get('kasa_no', '')).replace('.0', '')
                kasa_str = f"K{kasa_no}" if kasa_no and kasa_no != 'nan' else ""
                kamera_kontrol.append(f"✅ KAMERA BAK {tarih} {saat} {kasa_str}".strip())
                continue
        ...
        if kategori and kategori in kategori_urunleri:
            ...
            for alt_kod in kategori_urunleri[kategori]:
                if alt_kod != malzeme_kodu and alt_kod in iptal_data and iptal_data[alt_kod]:
                    iptaller = iptal_data[alt_kod]
                    son_iptal = iptaller[0] if iptaller else None
                    if son_iptal:
                        alt_ad = ""
                        ...
                        kamera_kontrol.append(f"🔄 KATEGORİ: {str(alt_ad)[:30]} → ✅ KAMERA BAK {tarih} {saat} {kasa_str}".strip())
                        bulundu = True
                        break
    ...
    result_df['KAMERA KONTROL DETAY'] = kamera_kontrol
    
    return result_df


def create_ic_hirsizlik_excel_surekli(df, magaza_kodu, magaza_adi):
    """Sürekli envanter iç hırsızlık Excel raporu"""
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "İç Hırsızlık Analizi"
    
    # Başlık
    ws['A1'] = f"SÜREKLİ ENVANTER - İÇ HIRSIZLIK ANALİZİ"
    ws['A1'].font = Font(bold=True, size=14)
    ws['A2'] = f"Mağaza: {magaza_kodu} - {magaza_adi}"
    ws['A3'] = f"Tarih: {datetime.now().strftime('%d.%m.%Y %H:%M')}"
    
    if len(df) == 0:
        ws['A5'] = "✅ İç hırsızlık şüphesi tespit edilmedi"
        output = BytesIO()
        wb.save(output)
        output.seek(0)
    ...
    # Veriler
    risk_fills = {
        '🔴 ÇOK YÜKSEK': PatternFill('solid', fgColor='FF4444'),
        '🟠 YÜKSEK': PatternFill('solid', fgColor='FF8800'),
        '🟡 ORTA': PatternFill('solid', fgColor='FFCC00')
    }
    ...
    for row_idx, (_, row) in enumerate(df.iterrows(), start=6):
        for col_idx, val in enumerate(row.values, 1):
            ...
            # Risk kolonunu renklendir
            if headers[col_idx-1] == 'Risk' and val in risk_fills:
